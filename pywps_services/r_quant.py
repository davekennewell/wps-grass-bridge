...
class r_quant(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.quant', title = 'Produces the quantization file for a floating-point map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.quant.html')
    ...
    self.addLiteralInput(identifier = 'fprange', title = 'Floating point range: dmin,dmax', minOccurs = 0, maxOccurs = 1, type = type("string"), allowedValues = '*')
    self.addLiteralInput(identifier = 'range', title = 'Integer range: min,max', minOccurs = 0, maxOccurs = 1, type = type("string"), allowedValues = '*')
