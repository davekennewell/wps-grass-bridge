...
class v_select(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.select', title = 'Selects features from vector map (A) by features from other vector map (B).', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'geometry'}, {'type': 'simple', 'title': 'spatial query'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.select.html')
    ...
    self.addLiteralInput(identifier = 'operator', title = 'A feature is written to output if the result of operation ainput operator binput is true. An input feature is considered to be true, if category of given layer is defined.', minOccurs = 1, maxOccurs = 1, type = type("string"), default = "overlap", allowedValues = ['overlap', 'equals', 'disjoint', 'intersects', 'touches', 'crosses', 'within', 'contains', 'overlaps', 'relate'])
    self.addLiteralInput(identifier = 'relate', title = 'Intersection Matrix Pattern used for relate operator', minOccurs = 0, maxOccurs = 1, type = type("string"), allowedValues = '*')
