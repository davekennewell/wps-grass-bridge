...
class v_out_vtk(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.out.vtk', title = 'Converts a vector map to VTK ASCII output.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'export'}, {'type': 'simple', 'title': 'VTK'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.out.vtk.html')
    ...
    self.addLiteralInput(identifier = '-n', title = 'Export numeric attribute table fields as VTK scalar variables', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])

    # complex outputs
    self.addComplexOutput(identifier = 'output', title = 'Name for output VTK file', formats = [{'mimeType': 'text/plain'}])
