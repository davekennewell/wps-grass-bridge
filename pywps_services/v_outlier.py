...
class v_outlier(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.outlier', title = 'Removes outliers from vector point data.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'statistics'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.outlier.html')
    ...
    self.addLiteralInput(identifier = 'soe', title = 'Interpolation spline step value in east direction', minOccurs = 0, maxOccurs = 1, type = type(0.0), default = 10.0)
    ...
    self.addComplexOutput(identifier = 'outlier', title = 'Name of output outlier vector map', formats = [{'mimeType': 'text/xml', 'schema': 'http://schemas.opengis.net/gml/2.1.2/feature.xsd', 'encoding': 'UTF-8'}, {'mimeType': 'text/xml', 'schema': 'http://schemas.opengis.net/kml/2.2.0/ogckml22.xsd', 'encoding': 'UTF-8'}])

    self.addComplexOutput(identifier = 'qgis', title = 'Name of vector map for visualization in QGIS', formats = [{'mimeType': 'text/xml', 'schema': 'http://schemas.opengis.net/gml/2.1.2/feature.xsd', 'encoding': 'UTF-8'}, {'mimeType': 'text/xml', 'schema': 'http://schemas.opengis.net/kml/2.2.0/ogckml22.xsd', 'encoding': 'UTF-8'}])

  def execute(self):
    starter = PyWPSGrassModuleStarter()
    starter.fromPyWPS("v.outlier", self.inputs, self.outputs, self.pywps)
