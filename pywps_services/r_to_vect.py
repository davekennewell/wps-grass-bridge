...
class r_to_vect(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addLiteralInput(identifier = 'feature', title = 'Feature type', minOccurs = 1, maxOccurs = 1, type = type("string"), default = "line", allowedValues = ['point', 'line', 'area'])
    ...
    self.addLiteralInput(identifier = '-z', title = 'Write raster values as z coordinate. Table is not created. Currently supported only for points.', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
    self.addLiteralInput(identifier = '-b', title = 'Do not build vector topology (recommended for massive point conversion)', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
