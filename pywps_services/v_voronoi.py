...
class v_voronoi(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addLiteralInput(identifier = 'smoothness', title = 'Applies to input areas only. Smaller values produce smoother output but can cause numerical instability.', minOccurs = 0, maxOccurs = 1, type = type(0.0), default = 0.25)
    self.addLiteralInput(identifier = 'thin', title = 'Applies only to skeleton extraction. Default = -1 will extract the center line.', minOccurs = 0, maxOccurs = 1, type = type(0.0), default = -1.0)
