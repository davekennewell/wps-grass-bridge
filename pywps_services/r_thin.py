...
class r_thin(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.thin', title = 'Thins non-zero cells that denote linear features in a raster map layer.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'geometry'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.thin.html')
    ...
    self.addLiteralInput(identifier = 'iterations', title = 'Maximal number of iterations', minOccurs = 0, maxOccurs = 1, type = type(0), default = 200)
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.thin", self.inputs, self.outputs, self.pywps)
