...
class r_series(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addComplexInput(identifier = 'file', title = 'Input file with raster map names and optional weights per line, field separator between name and weight is |', minOccurs = 0, maxOccurs = 1, formats = [{'mimeType': 'text/plain'}])
    self.addLiteralInput(identifier = 'method', title = 'Aggregate operation', minOccurs = 1, maxOccurs = 1024, type = type("string"), allowedValues = ['average', 'count', 'median', 'mode', 'minimum', 'min_raster', 'maximum', 'max_raster', 'stddev', 'range', 'sum', 'variance', 'diversity', 'slope', 'offset', 'detcoeff', 'quart1', 'quart3', 'perc90', 'quantile', 'skewness', 'kurtosis'])
