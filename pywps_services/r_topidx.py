...
class r_topidx(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.topidx', title = 'Creates topographic index map from elevation raster map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'hydrology'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.topidx.html')
    ...
    # complex outputs
    self.addComplexOutput(identifier = 'output', title = 'Name for output topographic index map', formats = [{'mimeType': 'image/tiff'}, {'mimeType': 'image/geotiff'}, {'mimeType': 'application/geotiff'}, {'mimeType': 'application/x-geotiff'}, {'mimeType': 'application/x-erdas-hfa'}, {'mimeType': 'application/netcdf'}, {'mimeType': 'application/x-netcdf'}])

  def execute(self):
    starter = PyWPSGrassModuleStarter()
    starter.fromPyWPS("r.topidx", self.inputs, self.outputs, self.pywps)
