...
class v_net_connectivity(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addLiteralInput(identifier = 'ncolumn', title = 'Name of node capacity column', minOccurs = 0, maxOccurs = 1, type = type("string"), allowedValues = '*')
    self.addLiteralInput(identifier = 'set1_layer', title = 'A single vector map can be connected to multiple database tables. This number determines which table to use. Layer name for direct OGR access.', minOccurs = 1, maxOccurs = 1, type = type("string"), default = "1")
    ...
    self.addLiteralInput(identifier = 'set2_layer', title = 'Set2 layer number or name', minOccurs = 1, maxOccurs = 1, type = type("string"), default = "1")
