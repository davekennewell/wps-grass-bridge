...
class r_buffer(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.buffer', title = 'Creates a raster map layer showing buffer zones surrounding cells that contain non-NULL category values.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'buffer'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.buffer.html')
    ...
    self.addLiteralInput(identifier = 'distances', title = 'Distance zone(s)', minOccurs = 1, maxOccurs = 1024, type = type(0.0), allowedValues = '*')
    self.addLiteralInput(identifier = 'units', title = 'Units of distance', minOccurs = 0, maxOccurs = 1, type = type("string"), default = "meters", allowedValues = ['meters', 'kilometers', 'feet', 'miles', 'nautmiles'])
    self.addLiteralInput(identifier = '-z', title = 'Ignore zero (0) data cells instead of NULL cells', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.buffer", self.inputs, self.outputs, self.pywps)
