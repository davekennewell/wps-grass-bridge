...
class r_transect(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.transect', title = 'Outputs raster map layer values lying along user defined transect line(s).', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'transect'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.transect.html')
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.transect", self.inputs, self.outputs, self.pywps)
