...
class r_cross(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addLiteralInput(identifier = '-z', title = 'Non-zero data only', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.cross", self.inputs, self.outputs, self.pywps)
