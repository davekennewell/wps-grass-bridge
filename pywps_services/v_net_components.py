...
class v_net_components(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.net.components', title = 'Computes strongly and weakly connected components in the network.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'network'}, {'type': 'simple', 'title': 'components'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.net.components.html')
    ...
    self.addLiteralInput(identifier = 'method', title = 'Type of components', minOccurs = 1, maxOccurs = 1, type = type("string"), allowedValues = ['weak', 'strong'])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("v.net.components", self.inputs, self.outputs, self.pywps)

if __name__ == "__main__":
  process = v_net_components()
  process.execute()
