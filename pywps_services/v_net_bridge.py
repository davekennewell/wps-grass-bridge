...
class v_net_bridge(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.net.bridge', title = 'Computes bridges and articulation points in the network.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'network'}, {'type': 'simple', 'title': 'articulation points'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.net.bridge.html')
    ...
    self.addLiteralInput(identifier = 'method', title = 'Feature type', minOccurs = 1, maxOccurs = 1, type = type("string"), allowedValues = ['bridge', 'articulation'])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("v.net.bridge", self.inputs, self.outputs, self.pywps)

if __name__ == "__main__":
  process = v_net_bridge()
  process.execute()
