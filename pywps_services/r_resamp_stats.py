...
class r_resamp_stats(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.resamp.stats', title = 'Resamples raster map layers to a coarser grid using aggregation.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'resample'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.resamp.stats.html')
    ...
    self.addLiteralInput(identifier = 'method', title = 'Aggregation method', minOccurs = 0, maxOccurs = 1, type = type("string"), default = "average", allowedValues = ['average', 'median', 'mode', 'minimum', 'maximum', 'quart1', 'quart3', 'perc90', 'sum', 'variance', 'stddev', 'quantile'])
    ...
    self.addLiteralInput(identifier = '-w', title = 'Weight according to area (slower)', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.resamp.stats", self.inputs, self.outputs, self.pywps)

if __name__ == "__main__":
  process = r_resamp_stats()
  process.execute()
