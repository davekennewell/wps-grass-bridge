...
class r_surf_idw(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.surf.idw', title = 'Surface interpolation utility for raster map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'surface'}, {'type': 'simple', 'title': 'interpolation'}, {'type': 'simple', 'title': 'IDW'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.surf.idw.html')
    ...
    self.addLiteralInput(identifier = '-e', title = 'Output is the interpolation error', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.surf.idw", self.inputs, self.outputs, self.pywps)
