...
class r_surf_idw2(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.surf.idw2', title = 'Surface generation program.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'surface'}, {'type': 'simple', 'title': 'interpolation'}, {'type': 'simple', 'title': 'IDW'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.surf.idw2.html')
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.surf.idw2", self.inputs, self.outputs, self.pywps)
