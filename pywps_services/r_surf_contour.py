...
class r_surf_contour(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.surf.contour', title = 'Generates surface raster map from rasterized contours.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'surface'}, {'type': 'simple', 'title': 'interpolation'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.surf.contour.html')

    # Literal and complex inputs
    self.addComplexInput(identifier = 'input', title = 'Name of input raster map containing contours', minOccurs = 1, maxOccurs = 1, formats = [{'mimeType': 'image/tiff'}, {'mimeType': 'image/geotiff'}, {'mimeType': 'application/geotiff'}, {'mimeType': 'application/x-geotiff'}, {'mimeType': 'image/png'}, {'mimeType': 'image/gif'}, {'mimeType': 'image/jpeg'}, {'mimeType': 'application/x-erdas-hfa'}, {'mimeType': 'application/netcdf'}, {'mimeType': 'application/x-netcdf'}])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.surf.contour", self.inputs, self.outputs, self.pywps)

if __name__ == "__main__":
  process = r_surf_contour()
  process.execute()
