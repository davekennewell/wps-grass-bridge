...
class v_to_points(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.to.points', title = 'Creates points along input lines in new vector map with 2 layers.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'geometry'}, {'type': 'simple', 'title': '3D'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.to.points.html')
    ...
    self.addLiteralInput(identifier = 'llayer', title = 'Vector features can have category values in different layers. This number determines which layer to use. When used with direct OGR access this is the layer name.', minOccurs = 0, maxOccurs = 1, type = type("string"), default = "1")
