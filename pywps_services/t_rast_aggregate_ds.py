...
class t_rast_aggregate_ds(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 't.rast.aggregate.ds', title = 'Aggregated data of an existing space time raster dataset using the temporal topology of a second space time dataset.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'temporal'}, {'type': 'simple', 'title': 'aggregation'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/t.rast.aggregate.ds.html')
