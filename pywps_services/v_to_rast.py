...
class v_to_rast(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.to.rast', title = 'Converts (rasterize) a vector map into a raster map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'conversion'}, {'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'rasterization'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.to.rast.html')
    ...
    self.addLiteralInput(identifier = '-d', title = 'All cells touched by the line will be set, not only those on the render path', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
