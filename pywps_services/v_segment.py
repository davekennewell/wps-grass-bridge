...
class v_segment(WPSProcess):
  ...
  def __init__(self):
    ...
    self.addComplexInput(identifier = 'file', title = '- for standard input', minOccurs = 0, maxOccurs = 1, formats = [{'mimeType': 'text/plain'}])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("v.segment", self.inputs, self.outputs, self.pywps)
