...
class v_net(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.net', title = 'Performs network maintenance.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'network'}, {'type': 'simple', 'title': 'network maintenance'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.net.html')
    ...
    self.addLiteralInput(identifier = '-s', title = 'For operation connect. By default, a new line from the point to the network is created.', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
