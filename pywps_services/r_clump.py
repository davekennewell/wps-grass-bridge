...
class r_clump(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.clump', title = 'Recategorizes data in a raster map by grouping cells that form physically discrete areas into unique categories.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'statistics'}, {'type': 'simple', 'title': 'reclass'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.clump.html')
    ...
    self.addLiteralInput(identifier = 'title', title = 'Title for output raster map', minOccurs = 0, maxOccurs = 1, type = type("string"), allowedValues = '*')
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.clump", self.inputs, self.outputs, self.pywps)
