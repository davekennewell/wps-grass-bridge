...
class v_hull(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.hull', title = 'Produces a 2D/3D convex hull for a given vector map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'geometry'}, {'type': 'simple', 'title': '3D'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.hull.html')
    ...
    self.addLiteralInput(identifier = '-r', title = 'Limit to current region', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
