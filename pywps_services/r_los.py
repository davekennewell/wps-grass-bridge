...
class r_los(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.los', title = 'Line-of-sight raster analysis program.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'viewshed'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.los.html')
    ...
    self.addLiteralInput(identifier = 'coordinate', title = 'Coordinate identifying the viewing position', minOccurs = 1, maxOccurs = 1, type = type("string"), allowedValues = '*')
    ...
    self.addLiteralInput(identifier = '-c', title = 'Consider earth curvature (current ellipsoid)', minOccurs = 0, maxOccurs = 1, type = type(True), default = False, allowedValues = [True, False])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.los", self.inputs, self.outputs, self.pywps)
