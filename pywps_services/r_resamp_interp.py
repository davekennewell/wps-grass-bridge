...
class r_resamp_interp(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.resamp.interp', title = 'Resamples raster map layers to a finer grid using interpolation.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'resample'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.resamp.interp.html')
    ...
    self.addLiteralInput(identifier = 'method', title = 'Sampling interpolation method', minOccurs = 0, maxOccurs = 1, type = type("string"), default = "linear", allowedValues = ['nearest', 'linear', 'cubic', 'lanczos'])
  ...
  def execute(self):
    ...
    starter.fromPyWPS("r.resamp.interp", self.inputs, self.outputs, self.pywps)

if __name__ == "__main__":
  process = r_resamp_interp()
  process.execute()
