...
class v_neighbors(WPSProcess):

  def __init__(self):
    WPSProcess.__init__(self, identifier = 'v.neighbors', title = 'Makes each cell value a function of the attribute values assigned to the vector points or centroids around it, and stores new cell values in an output raster map.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'vector'}, {'type': 'simple', 'title': 'algebra'}, {'type': 'simple', 'title': 'statistics'}, {'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'aggregation'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/v.neighbors.html')
    ...
    self.addLiteralInput(identifier = 'method', title = 'Neighborhood operation', minOccurs = 1, maxOccurs = 1, type = type("string"), default = "count", allowedValues = ['count'])
    self.addLiteralInput(identifier = 'size', title = 'Neighborhood diameter in map units', minOccurs = 1, maxOccurs = 1, type = type(0.0), allowedValues = '*')
  ...
  def execute(self):
    ...
    starter.fromPyWPS("v.neighbors", self.inputs, self.outputs, self.pywps)
