...
class r_water_outlet(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.water.outlet', title = 'Creates watershed basins.', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'hydrology'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.water.outlet.html')
    ...
    self.addLiteralInput(identifier = 'easting', title = 'Easting grid coordinates', minOccurs = 1, maxOccurs = 1, type = type("string"), allowedValues = '*')
    self.addLiteralInput(identifier = 'northing', title = 'Northing grid coordinates', minOccurs = 1, maxOccurs = 1, type = type("string"), allowedValues = '*')
