...
class r_fillnulls(WPSProcess):
  ...
  def __init__(self):
    WPSProcess.__init__(self, identifier = 'r.fillnulls', title = 'Fills no-data areas in raster maps using v.surf.rst splines interpolation', version = 1, statusSupported = True, storeSupported = True, metadata = [{'type': 'simple', 'title': 'raster'}, {'type': 'simple', 'title': 'elevation'}, {'type': 'simple', 'title': 'interpolation'}], abstract = 'http://grass.osgeo.org/grass70/manuals/html70_user/r.fillnulls.html')

    # Literal and complex inputs
    self.addComplexInput(identifier = 'input', title = 'Raster map in which to fill nulls', minOccurs = 1, maxOccurs = 1, formats = [{'mimeType': 'image/tiff'}, {'mimeType': 'image/geotiff'}, {'mimeType': 'application/geotiff'}, {'mimeType': 'application/x-geotiff'}, {'mimeType': 'image/png'}, {'mimeType': 'image/gif'}, {'mimeType': 'image/jpeg'}, {'mimeType': 'application/x-erdas-hfa'}, {'mimeType': 'application/netcdf'}, {'mimeType': 'application/x-netcdf'}])
    ...
    # complex outputs
    self.addComplexOutput(identifier = 'output', title = 'Output raster map with nulls filled by interpolation from surrounding values', formats = [{'mimeType': 'image/tiff'}, {'mimeType': 'image/geotiff'}, {'mimeType': 'application/geotiff'}, {'mimeType': 'application/x-geotiff'}, {'mimeType': 'application/x-erdas-hfa'}, {'mimeType': 'application/netcdf'}, {'mimeType': 'application/x-netcdf'}])
